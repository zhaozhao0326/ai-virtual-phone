  ...
  try {
    ...
    if (action === "thumb") {
      ...
      const owner = await supabaseFetch<unknown[]>(
        `${table}?id=eq.${encodeFilter(id)}&deleted_at=is.null&select=id,author_id,cover,updated_at&limit=1`,      );      if (!owner.ok) return NextResponse.json({ ok: false, error: owner.error }, { status: owner.status });
      ...
      const written = await supabaseFetch<unknown[]>(
        ...
        { method: "PATCH", headers: { Prefer: "return=minimal" }, body: JSON.stringify(patch) },      );      if (!written.ok) return NextResponse.json({ ok: false, error: written.error }, { status: written.status });
