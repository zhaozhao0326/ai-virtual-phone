...
export function ChatRoom({ session, onBack }: ChatRoomProps) {
    ...
    const processGroupParts = async (
        ...
        for (const r of results) {
            ...
            const { parts: rawParts, stateValues, freshStateValues, statusPanel, innerMonologue } = parseAIResponse(r.responseText, getCurrentStateForCharacter(r.characterId), { allowImageTags });
            ...
            if (session.isGroup) {
                ...
                const results = await generateGroupChatCompletion(session, latestMessages, {
                    ...
                    onTextPart: async (text, senderInfo, options) => {
                        ...
                        const { parts: rawParts, stateValues, freshStateValues, statusPanel, innerMonologue } = parseAIResponse(text, previousState, { allowImageTags });
