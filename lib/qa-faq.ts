...
### Q: 小坊回复到一半突然停了是怎么回事？
几种常见情况，现在都会给出明确提示：① 单轮对话的工具调用轮数用尽（上限 8 轮，连续读多页指南/做多步操作时可能碰到），提示后回复「继续」即可接着做；② 模型输出长度上限把内容截断（写大型 APP 时最常见），回复「继续」重试，经常出现就到「设置 → 预设」调大最大输出 token；③ 输出中途停顿几分钟后一口气全出：流式连接停滞（常见于中转站开了响应缓冲），停滞超 90 秒会自动中断并改用整段获取，消息里会显示切换说明——需等完整生成后一次性显示，属正常降级；④ 正在生成大段工具调用（如整个 APP 的代码）时会显示「正在编写工具调用…」，不是卡死。

### Q: 让小坊写大型 APP 总是失败 / 写一半被截断？
两个常见原因：① 模型单次回复有最大输出 token 上限（中转站往往只有几千），大 APP 的完整 HTML 一次写不完——小坊支持分段写入：用「暂存应用文件」分多轮追加写 HTML，最后「安装暂存应用」组包安装，任何输出上限都不受影响；也可到「设置 → 预设」调大最大输出 token。② 屏幕上出现 <|open|>call tool= 之类的乱码：说明该 API 渠道不支持原生工具调用（没把模型的内部工具格式翻译成标准结构），到「设置 → API 设置」把该配置的「原生工具」开关关掉，工坊会改走文本协议，任何模型都能用；或更换正规渠道。

### Q: 小坊装的 APP 能带权限声明和聊天工具（chat.tools）吗？
能。简单场景：「安装本机应用」可传 permissions 参数覆盖默认权限。完整场景：用「暂存应用文件」写入 manifest.json（可声明 permissions、extensions.tools 等）+ 入口 HTML + presets.json 等文件，再「安装暂存应用」——等价于上传完整 zip 应用包，manifest 声明全部生效。
