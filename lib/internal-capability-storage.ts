...
const TIMED_WAKE_PARAMETER_SCHEMA = JSON.stringify({
    ...
    properties: {
        delayMinutes: {
            type: "number",
            description: "从现在开始多少分钟后到点（到点你会被带回来主动联系）。至少 30 分钟；几分钟就再找对方属于打扰，请按真实生活节奏安排",
        },
        intent: {
            type: "string",
...
const TIMED_WAKE_USAGE_GUIDE = [
    ...
    "",
    "参数：",
    "- delayMinutes (number): 从现在开始多少分钟后到点。**至少 30 分钟**。隔几分钟就找对方属于打扰，请按真实生活节奏安排（常见 1~24 小时）。",
    ...
    "- 对方一直没回时不要反复续约：没得到回应最多再主动找一两次就停，把说话的主动权留给对方。",
    "",
    "示例：",
    '[执行动作:稍后主动联系({"delayMinutes":120,"intent":"过两小时看看对方忙完了没，如果还合适就轻轻找一句"})]',
].join("\n");
