...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    title: "表情包：合集图会自动拆开",
    highlights: [
      "批量导入表情包时，选中的如果是一张「合集图」（一张图里排着好几个表情），会自动认出每个表情的位置，分别切成独立表情",
      "拆出来的每个表情都能单独改名、单独删除，和手动添加的表情一样用",
      "识别全在本地完成，不联网、不消耗任何额度；认错了可以关掉「合集自动拆分」，按原来的整张图导入",
      "单个添加时选了合集图，会提示「拆成 N 个表情」，点一下转到批量导入去拆（那里能逐个改名）",
    ],
  },
  {
    version: "1.7.89",
    date: "2026-09-18",
    title: "线下模式：角色发言可以听出来了",
    highlights: [
      "线下模式每条角色回复多了一个朗读小按钮：点一下用角色自己的声音把台词念出来，再点一下停",
      "单聊线下用该角色自己绑定的嗓音朗读；群聊线下是第三人称连续叙事（模型不标发言者），改为整段用中性嗓音朗读，避免把台词错配成别人的声音",
      "角色没有单独绑语音时自动用设备自带的中文朗读兜底，不额外花钱",
      "朗读走角色已有的语音配置（如已挂的 MiniMax 声音），点一下才生成一次，不听不消耗",
    ],
  },
  {
    version: "1.7.88",
    date: "2026-09-17",
    title: "世界书：单独上传 JSON 也能认了",
    highlights: [
      "「设置 → 世界书 → 导入世界书 (JSON)」现在能直接认其他工具导出的世界书文件，也能认角色卡文件（取卡里内嵌的那本世界书），不再一律提示「不支持该世界书格式」",
      "同一本书夹在角色卡里能导入、单独上传却进不来，是因为上传口过去只认本应用自己导出的格式；现在两条路都用同一套解析，触发词补全、常驻、注入位置、分类的做法完全一致",
      "一个文件导入成一本书，分类保留在每条条目上；本应用自己导出的世界书格式行为不变，已导入的世界书不受影响",
    ],
  },
  {
    version: "1.7.87",
    date: "2026-09-17",
    title: "线下模式：每轮回复末尾多一个小标签，点开是场景手记",
    highlights: [
      "线下模式每个回合末尾多一个小标签，点开浮出一块场景手记：天气氛围、所在位置、对角色的印象、正在做什么、情绪、此刻心声、下一步、关系阶段与变化。看完关掉，不占聊天版面",
      "每轮的「摘要」并进同一块里，不再单独占一行——摘要照旧写进短期记忆，只是显示位置挪到了这块浮层",
      "卡片沿用与线上自定义状态栏同一套机制（输出契约 + 沙盒渲染），在聊天信息页即可编辑；没改过就用内置的手记模板",
      "状态栏内容默认不生成，在聊天信息页的「线下模式状态栏」单独开启。开启后仍是一轮一次调用，只多一点点输出字数；没开时标签里只有摘要",
      "已生成的手记会一直留在历史记录里可看，关掉开关也不会丢，只是不再生成新的",
    ],
  },
  {
    version: "1.7.86",
  ...
  },
  {
    ...
    title: "人设与保存保真修复",
    highlights: [
      "修复：设定内容过长时可能导致角色人设整段丢失、回复不按人设的问题；现在改为优先保留人设，只精简靠后的设定内容",
      "修复：少数情况下，刚保存的编辑（如人设修改）在重新打开后可能被回滚成旧版本",
      "以上修复不改变已有功能的开关与用法，无需额外设置",
    ],
  },
  {
    version: "1.7.74",
    ...
    title: "资料库调用收口",
    highlights: [
      "修复：角色偶尔会撇开戏、反复翻看手机里的原始资料，导致回话跑题、答非所问；现在这条路已收口",
      "「查手机」这类内置工作流照常可用，不受影响",
      "资料库查询加限流：单次对话内限制查询次数，重复的同一查询直接拦截；返回内容也已精简",
      "收敛资料库动作的使用说明：仅在你明确要求查看本地数据时才使用，日常聊天与角色扮演不主动翻看",
      "群聊、工具箱、关系网等由指令触发的功能行为不变",
    ],
  },
  {
    version: "1.7.72",
  ...
  },
  {
    ...
    title: "角色卡导入按分类拆解",
    highlights: [
      "导入角色卡时，内嵌世界书按原卡的分类/文件夹拆成多本，不再整本挤成一册",
      "整本设定被压成一条超长内容时，按标题层级自动切成多条并归入对应分类",
      "没有触发词的世界书条目自动补全关键词；实在补不出的才设为常驻，避免导入了却永不生效",
      "角色卡自带的正则脚本、预设、系统提示与追加指令一并导入；正则自动绑定该角色，预设不自动启用",
      "独家特调导入同源角色卡时，世界书同样按分类拆成多件基底，正则转为滤网材料",
    ],
  },
  {
    version: "1.7.66",
  ...
  },
  {
    ...
    title: "追问链防刷屏",
    highlights: [
      "焦虑值驱动的自动追问新增总开关，可在追发设置中完全关闭",
      "连续追问上限收紧、最短间隔拉长，避免角色反复追发消息",
    ],
  },
  {
    version: "1.7.63",
    ...
    title: "删除角色彻底清理",
    highlights: [
      "删除角色时一并清除其聊天会话与定时唤醒、冷场重连等排期，避免角色已删除仍自动发消息",
    ],
  },
  {
    version: "1.7.62",
    ...
    title: "群暖场有来有回",
    highlights: [
      "群冷场暖场默认带一小段「起头 + 接话」，像真的聊起来了；可在该群设置里关闭，改回只发一句",
      "单次暖场消息数量设上限，避免刷屏",
    ],
  },
  {
  ...
  },
  {
    version: "1.7.58",
    date: "2026-08-30",
    title: "自主互动全场景感知真实时间差",
    highlights: [
      "1:1 角色主动私聊（含记忆关心）补强时间感知：隔了很久再主动找用户，不会假装时间没流逝、接着旧话题当连续对话",
      "群冷场暖场与离线剧情推进互不干扰：暖场只是真实群的叠加层，离线模式原有的自主推进剧情能力始终保留",
      "所有自主互动均在触发时由模型感知时间间隔，不新增后台轮询、不额外消耗 token",
    ],
  },
  {
    version: "1.7.57",
    date: "2026-08-30",
    title: "自主互动感知真实时间差（避免跨天跳戏）",
    highlights: [
      "群冷场暖场现在会感知真实时间差：隔了几天再暖场，不会接着上次的旧话题当连续对话说",
      "1:1 冷场重连同样补强：明确提示角色真实时间已过去，可自然隔几天重提旧事或直接开新话题",
      "仅在触发自主互动时由模型感知时间间隔，不新增后台轮询、不额外消耗 token",
    ],
  },
  {
    version: "1.7.56",
    date: "2026-08-29",
    title: "群冷场自动暖场（可选互动频率）",
    highlights: [
      "群设置新增「冷场自动暖场」：可给每个群设互动频率（如 6 小时 / 24 小时），群太久没消息时由允许的角色主动接话暖场",
      "总开关默认关闭，且需逐个勾选允许暖场的角色（选人白名单），绝不默认全员；有互动不强制触发",
      "角色有强烈分享欲等特殊触发仍随时可发，不受频率限制；过安静时段不打扰",
    ],
  },
  {
    version: "1.7.55",
    ...
    title: "群聊线下剧情可落成真实群聊",
    highlights: [
      "群聊线下模式新增「落成真实群」：一键把剧本里角色发言抽成真实气泡发进对应群聊",
      "落成前可预览编辑每条气泡、改发言者或跳过，确认后才落库，不污染原剧情",
      "抽取仅在手动触发时调用一次模型，不后台轮询、不自动同步，安静可控",
    ],
  },
  {
    version: "1.7.54",
  ...
  },
  {
    version: "1.7.53",
    date: "2026-08-29",
    title: "NAI 生图模型新增 V5 选项",
    highlights: [
      "图像生成设置新增 NovelAI Diffusion V5（Curated / Full）两个可选模型",
      "模型下拉更新至官方最新矩阵（V5 / V4.5 / V4 / V3），标识与官方 API 一致",
      "默认模型保持不变，用户可在设置中手动切换至 V5；V5 算力消耗更高，按需选用",
    ],
  },
  {
    version: "1.7.52",
  ...
  },
  {
    version: "1.7.46",
    date: "2026-08-26",
    title: "恢复梦境功能（可开关），日记保持停用",
    highlights: [
      "恢复梦境自动生成：信箱「梦境」页新增开关与角色多选，默认关闭，仅对选中角色生效",
      "日记不再自动生成，保持停用",
    ],
  },
  {
  ...
  },
  {
    version: "1.7.43",
    date: "2026-08-26",
    title: "梦境增设全局开关与选人，默认关闭以控制后台调用量",
    highlights: [
      "新增梦境（含日记）全局开关，默认关闭：后台不再对全部角色批量生成梦境/日记，解决单次发消息触发大量调用的问题",
      "信箱「梦境」页新增开关与角色多选：开启后仅对选中的角色生成，未选中的角色不会做梦",
      "保留单角色级开关（角色档案内可单独关闭）作为补充控制",
    ],
  },
  {
    version: "1.7.42",
  ...
  },
  {
    version: "1.7.38",
    date: "2026-08-25",
    title: "修复线下模式正文段落挤成一团",
    highlights: [
      "修复：线下模式输出的小说正文所有段落被压成一段，阅读时字全融在一起",
      "解析时保留换行与段落空行，只压缩行内多余空格",
    ],
  },
  {
    version: "1.7.37",
  ...
  },
  {
    version: "1.7.31",
    date: "2026-08-25",
    title: "修复文字生图的脸：画面里有角色时，参考图不再认错人",
    highlights: [
      "修复：文字生图勾选多个角色时，只要其中一人没有参考图，后面的脸就会和参考图错位（锁错脸、脸不像）",
      "现在每张参考图严格对应各自的主人，有参考图的人脸更稳",
    ],
  },
  {
    version: "1.7.30",
